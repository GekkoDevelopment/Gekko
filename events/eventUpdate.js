...
module.exports = {
    ...
    async execute(oldEvent, newEvent) {
        const logChannelId = await MySQL.getValueFromTableWithCondition('guilds', 'logging_channel', 'guild_id', newEvent.guild.id);
        ...
        if (oldEvent.name !== newEvent.name) {
            ...
            .setDescription(`> **Event Name:** ${newEvent.name} \n> **Where:** <#${newEvent.channelId}> \n> **When:** <t:${Math.floor(newEvent.scheduledStartTimestamp / 1000)}:R> \n> **Created by:** <@${executor.id}> \n> \n> **Event Description:** \n> ${newEvent.description}`)
            .setColor('Orange')
            .setTimestamp()
            .setFooter({ text: 'Gekkō', iconURL: newEvent.client.user.displayAvatarURL() });
            ...
            embed.addFields(
                {
                    ...
                    value: `<t:${Math.floor(oldEvent.scheduledStartTimestamp / 1000)}:F>`,
                    inline: true
                },
                {
                    name: 'New Start Time:',
