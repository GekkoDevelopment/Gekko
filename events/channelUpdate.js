const { Events, EmbedBuilder, AuditLogEvent, NewsChannel } = require('discord.js');
...
module.exports = {
    ...
    async execute(oldChannel, newChannel) {
        const loggingType = await MySQL.getValueFromTableWithCondition('guilds', 'logging_type', 'guild_id', newChannel.guild.id);
        ...
        if (!loggingType) {
            ...
            const logChannelId = await MySQL.getValueFromTableWithCondition('guilds', 'logging_channel', 'guild_id', newChannel.guild.id);
            ...
            if (newChannel.type === 0) {
                ...
                if (oldChannel.name !== newChannel.name) {
                    ...
                    .setDescription(`> **Channel Name:** <#${newChannel.id}> (||${newChannel.id}||) \n> **Updated by:** <@${executor.id}>`)
                    .setColor('Orange')
                    .setTimestamp()
                    .setFooter({ text: 'Gekkō', iconURL: newChannel.client.user.displayAvatarURL() });
            ...
            if (newChannel.type === 15) {
                ...
                if (oldChannel.availableTags !== newChannel.availableTags) {
                    const oldTags = oldChannel.availableTags.length > 0 ? oldChannel.availableTags.map(tag => `${tag.name}`).join(', ') : 'None';
                    const newTags = newChannel.availableTags.length > 0 ? newChannel.availableTags.map(tag => `${tag.name}`).join(', ') : 'None';
