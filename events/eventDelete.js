...
module.exports = {
    ...
    async execute(event) {
        const logChannelId = await MySQL.getValueFromTableWithCondition('guilds', 'logging_channel', 'guild_id', event.guild.id);
        ...
        .setDescription(`> **Event Name:** ${event.name} \n> **Where:** <#${event.channelId}> \n> **When:** <t:${Math.floor(event. scheduledStartTimestamp / 1000)}:R> \n> **Deleted by:** <@${executor.id}> \n> \n> **Event Description:** \n> ${event.description}`)
        .setColor('Red')
        .setTimestamp()
        .setFooter({ text: 'Gekkō', iconURL: event.client.user.displayAvatarURL() });
