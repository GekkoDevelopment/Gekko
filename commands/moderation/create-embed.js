...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option => option.setName('description').setDescription('Set the description of the embed.').setRequired(true))
        .addStringOption(option => option.setName('color').setDescription('The color of the embed. (you can use hex code too!)'))
        .addStringOption(option => option.setName('field-name').setDescription('The field name of the embed.'))
        .addStringOption(option => option.setName('field-value').setDescription('The field value of the embed.'))
    ...
    async execute(interaction) {
        ...
        if (image) {
            if (!image.startsWith('http')) {
                return await interaction.reply({ content: "You can't use make this your image! (Reason: Starts with HTTP)", ephemeral: true });
            }
        }

        if (thumbnailUrl) {
            if (!thumbnailUrl.startsWith('http')) {
                return await interaction.reply({ content: "You can't use make this your thumbnail! (Reason: Starts with HTTP)", ephemeral: true });
            }
        }
        ...
        await interaction.reply({ content: 'Your embed has sent below!', ephemeral: true });
